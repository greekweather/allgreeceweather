...
async function compressImage(file: File): Promise<File> {
	if (!["image/jpeg", "image/png", "image/webp"].includes(file.type))
		throw new Error("Χρησιμοποίησε JPG, PNG ή WebP.");
	...
	if (blob.size > MAX_OUTPUT_BYTES)
		throw new Error("Η εικόνα παραμένει πολύ μεγάλη μετά τη συμπίεση.");
...
export function ImageUpload({ onUploaded }: { onUploaded: (url: string, alt: string) => void }) {
	...
	return (
		<div className="form-field">
			...
			<small>
				{status ||
					"JPG/PNG/WebP έως 8 MB αρχικά· το σύστημα αποθηκεύει βελτιστοποιημένο WebP έως περίπου 800 KB."}
			</small>
		</div>
	);
}
